...
namespace CV_app
{
    public class GxBitmap
    {
        ...
        private PictureBox m_pic_ShowImage = null;                ///<图片显示控件
        private bool m_bIsColor = false;               ///<是否支持彩色相机
        private byte[] m_byMonoBuffer = null;                ///<黑白相机buffer
        private byte[] m_byColorBuffer = null;                ///<彩色相机buffer
        private byte[] m_byRawBuffer = null;                ///<用于存储Raw图的Buffer
        private int m_nPayloadSize = 0;                   ///<图像数据大小
        private int m_nWidth = 0;                   ///<图像宽度
        private int m_nHeigh = 0;                   ///<图像高度
        private Bitmap m_bitmapForSave = null;                ///<bitmap对象,仅供存储图像使用
