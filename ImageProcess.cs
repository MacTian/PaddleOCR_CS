    ...
    internal class ImageProcess
    {
        ...
        public Mat process(Mat img)
        {
            try
            {
                ...
                //CvInvoke.Threshold(region, region, 0, 255, ThresholdType.Otsu);
                if (binary == "true")
                {
                ...
                CvInvoke.Erode(img, img, element1, new Point(-1, -1), 1, BorderType.Default, new MCvScalar());//腐蚀
                CvInvoke.Dilate(img, img, element2, new Point(-1, -1), 1, BorderType.Default, new MCvScalar());//膨胀
            }
            catch (Exception ex)
            {
